...
export function ActionButtons({ product, selectedSize, selectedColor }: ActionButtonsProps) {
  const handleWhatsAppRedirect = () => {
    const { Name, Whatsapp_message } = product;
    
    // Check if selected size is custom based on the text content
    const isCustomSize = selectedSize && (
      selectedSize.toLowerCase().includes('custom') || 
      selectedSize.toLowerCase().includes('mtm') ||
      selectedSize.toLowerCase().includes('made to measure')
    );
    ...
    if (isCustomSize) {
      ...
      // Check if Whatsapp_message exists and is not empty or test data
      if (Whatsapp_message && 
  ...
  return (
    <div className="space-y-4 pt-8">
      ...
      <button 
        onClick={handleWhatsAppRedirect}
        className="w-full bg-white text-black py-4 px-8 font-medium text-lg uppercase tracking-wider border-2 border-black hover:bg-black hover:text-white transition-all duration-200 flex items-center justify-center space-x-2"
      >
